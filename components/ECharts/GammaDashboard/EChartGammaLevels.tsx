...
import { EChartGamma_Levels_Opts } from './EChartGamma_Levels_Opts';
import { EChartThemed } from '../EChartThemed';
import useCustomSWR from '@/lib/fetchdata/fetch-custom';
import { GAMMA_DASHBOARD_LEVELS_URL } from '@/lib/fetchdata/apiURLs';
import MainLoading from '@/app/loading';

interface EChartGammaLevelsProps {
  params: {
    und_symbol: string;
    date: string;
    exp_filter: string;
...
const EChartGammaLevels = ({ params }: EChartGammaLevelsProps) => {
  ...
  const { data, isLoading } = useCustomSWR(GAMMA_DASHBOARD_LEVELS_URL, params, {
    refreshInterval,
  });
  ...
  const ecOptions = EChartGamma_Levels_Opts(data.data);
...
export default EChartGammaLevels;
